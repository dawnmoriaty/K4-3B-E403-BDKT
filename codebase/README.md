...
- Mock: external web search and the learner feedback workflow. CP2 feedback records are stored in browser `localStorage` with `route_origin=grounded` or `route_origin=no_grounding`; they are not sent to a production Dev backend.
